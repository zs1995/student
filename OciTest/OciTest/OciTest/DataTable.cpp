...
// 	}
	...
	OCIHandleFree((dvoid *) m_phStmt, OCI_HTYPE_STMT);
	//OCIHandleFree(m_phStmt, OCI_DEFAULT);
}
...
char* CDataTable::GetDate(int iField,char* pDate)
{
	...
	if (iCentury < 0)	//公元前
	{
		iCentury = -iCentury;
		iYear = -iYear;
		sprintf(pDate,"BCE %02d%02d-%02d-%02d %02d:%02d:%02d",iCentury,iYear,iMonth,iDate,iHour,iMinute,iSecond);
	} else {
		sprintf(pDate,"%02d%02d-%02d-%02d %02d:%02d:%02d",iCentury,iYear,iMonth,iDate,iHour,iMinute,iSecond);
	}
	return pDate;
}
