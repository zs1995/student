...
//创建连接池
bool CDbPool::CreatePool(char* strDB,char* strUser,char* strPwd)
{
	try
	{
		int iLen = 60;
		strcpy_s((char*)pDatabase,iLen,strDB);

		strcpy_s((char*)pUserName,iLen,strUser);

		strcpy_s((char*)pPassWord,iLen,strPwd);
...
//		Loger.Log("Error - OCI_SUCCESS_WITH_INFO\n");
		break;
	case OCI_NEED_DATA:
//		Loger.Log("Error - OCI_NEED_DATA\n");
		throw 1;
		break;
	case OCI_NO_DATA:
		//		Loger.Log("Error - OCI_NODATA\n");
		break;
	case OCI_ERROR:
		OCIErrorGet(errhp,(ub4)1,(text*)NULL,&m_s_nErrCode,(OraText*)m_s_szErr,512,OCI_HTYPE_ERROR);
//		sprintf_s(strLog,1024,"Error - OCI_ERROR\n\tErrorCode:%d\n\tErrorInfo%s",m_s_nErrCode,m_s_szErr);
//		Loger.Log(strLog);
		throw 1;
		break;
	case OCI_INVALID_HANDLE:
//		Loger.Log("Error - OCI_INVALID_HANDLE\n");
		throw 1;
		break;
	case OCI_STILL_EXECUTING:
//		Loger.Log("Error - OCI_STILL_EXECUTE\n");
		throw 1;
		break;
	case OCI_CONTINUE:
//		Loger.Log("Error - OCI_CONTINUE\n");
		throw 1;
		break;
	default:
		break;
